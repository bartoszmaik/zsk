<!DOCTYPE html>
<html lang="pl">
  <head>
    <meta charset="utf-8">
    <meta name="author" content="Bartosz Maik">
    <meta name="keywords" content="angielski, kurs, angielski zawodowy, nauka języka, ponglish">
    <meta name="description" content="Ponglish - strona do nauki języka zawodowego">
    <meta name="robots" content="follow"/>
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <title>Kontakt</title>
    <link rel="stylesheet" href="css/bootstrap.min.css">
    <link rel="stylesheet" href="css/style.css">
  </head>

  <body>
    <header>
      <nav class="navbar shadow navbar-dark navbar-expand-lg">
          <a class="navabar-brand" href="index.php">
            <img src="images/logo.png" id="logo" class="d-inline-block align-middle" alt="logo">
            PONGLISH
          </a>

          <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#menu">
            <span class="navbar-toggler-icon"></span>
          </button>

          <div class="collapse navbar-collapse" id="menu">
              <ul class="navbar-nav">

                <li class="nav-item">
                  <a class="nav-link" href="index.php">Strona główna</a>
                </li>

                <li class="nav-item">
                  <a class="nav-link" href="learn.php">Ucz się</a>
                </li>

                <li class="nav-item">
                  <a class="nav-link" href="contact.php">Kontakt</a>
                </li>

                <li class="nav-item">
                  <a class="nav-link" href="account.php">Twoje konto</a>
                </li>

                <li class="nav-item">
                  <a class="nav-link active" href="login.php">Zaloguj się</a>
                </li>

              </ul>
          </div>
      </nav>
    </header>
<div class="container-fluid">
    <div class="row">

    <div class="left col-sm-12 col-md-6">
      <h1 class="mbot center">Zaloguj się!</h1>

          <form class="center border1 border-dark rounded" name="login" action="index.html" method="post">
              <div class="form-group">
                <label for="login">Login</label>
                <input type="text" class="form-control-lg" id="login" name="login">
              </div>

              <div class="form-group">
                <label for="password">Hasło</label>
                <input type="password" class="form-control-lg" id ="password" name="password">
              </div>
                <button type="button" class="mtop btn btn-dark">Zaloguj</button>

          </form>
        </div>

        <div class="right col-sm-12 col-md-6">
          <h1 class="mbot center">Nie masz konta? Zarejestruj się!</h1>

          <form class="center border1 border-dark rounded" name="register" action="index.html" method="post">
            <div class="form-group">
              <label for="login">Login</label>
              <input type="text" class="form-control-lg" id="login" name="login">
            </div>
            <div class="form-group">
              <label for="password">Hasło</label>
              <input type="password" class="form-control-lg" id="password" name="password">
            </div>
            <div class="form-group">
              <label for="password2">Powtórz hasło</label>
              <input type="password" class="form-control-lg" id="password2" name="password2">
            </div>
            <div class="form-group">
              <label for="email">E-mail</label>
              <input type="email" class="form-control-lg" id="email" name="email">
            </div>
              <button type="button" class="mtop btn btn-dark">Utwórz nowe konto</button>

          </form>
  </div>

  </div>
    </div>


    <footer>
      Stronę wykonał: Bartosz Maik 4C * Tu jakieś info random na razie, ale będzie inne *
    </footer>

    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
  	<script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.3/umd/popper.min.js" integrity="sha384-ZMP7rVo3mIykV+2+9J3UJ46jBk0WLaUAdn689aCwoqbBJiSnjAK/l8WvCWPIPm49" crossorigin="anonymous"></script>
  	<script src="js/bootstrap.min.js"></script>
  </body>
</html>
